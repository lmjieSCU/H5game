...
class SoundCtrl {
    ...
    public changeForbidden() {
        ...
        if (this.isForbidden) {
            ...
            let bgsound = GameData.I.stageInfo(GameData.I.level).length >= 10 ? "bossbgLv" : "bgsound";
            this.playbgSound(bgsound);
        }
    }

    public playSound(name: string, type: string = "mp3"): void {
        if (this.isForbidden) {
            return;
        }
        // this.stopMusic();
    ...
    public playbgSound(name: string, type: string = "mp3"): void {
        if (this.isForbidden) {
            return;
        }
        this.stopBgMusic();
