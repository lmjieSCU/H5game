...
module zmovie {
    ...
    export class ZMovieClip extends egret.DisplayObjectContainer {
        ...
        public isBodyFlag : boolean = false;
        ...
        public constructor(imgArr: any, libObj: any, mcName: string = null, scale_: number = 1, isBody:boolean=false) {
            super();
            this.isBodyFlag = isBody;
        ...
        public setMovieObject(imgArr: any, libObj: any, mcName: string, scale_: number = 1): void {
            ...
            for (var i: number = 0; i < len; i++) {
                ...
                var f_arr: any[] = arr[i];                      //取出每个layer
                var f_len: number = f_arr.length;   
                var pcfo: any = null;
                var k: number;

                for (var j: number = 0; j < f_len; j++) {
                    var cfo: any = f_arr[j];                //取出每个layer的{}
                    var obj3d: any = null;
        ...
        public play(): void {
            ...
            if (!this.isPlay) {
                this.isPlay = true;
                //if (!GameScene.instance.startEnterFlag)
       ...
       //
            ...
            if (typeof (frame) == "string") {
                ...
                if (null != this.mcObj.frameLabels) {
                    ...
                    if (null != labelObj) {
                        ...
                        // console.log(frame,this.beginFrame,this.endFrame)//跳招 97 110     跳招2 138 177  砍技2 473 537
        ...
        private gotoAndPlayLabel(label: string, isPlayToEnd: boolean = false): any {
            var labelObj: any;
            if (null != this.mcObj.frameLabels) {
                labelObj = this.mcObj.frameLabels[label];   //可获得对应label的beginframe与endframe
                if (null != labelObj) {
                    ...
                    // console.log(this.beginFrame,this.endFrame)
        ...
        public gotoAndPlay(frame: number | string): void {
            if (typeof (frame) == "string") {           //以label进入
                this.gotoAndPlayLabel(frame);
            } else {                                    //以帧进入
                frame = frame -1;
                this.currFrame = frame-1;
                this.parentPlayTime = 0;
                        ...
                        if (null == cfo.libName) {
                            ...
                            if (null == t) {
                                ...
                                if (null == this.libObj.mcObjs[cfo.libName]) {
                                    ...
                                    if (null == mc) {
                                        mc = new ZMovieClip(this.imgArr, this.libObj, cfo.libName, 1, this.isBodyFlag);
                                        mc.touchEnabled = false;
                                        mc.isBodyFlag = this.isBodyFlag;
                                        l_obj.mc = mc;
                                        mc.img_depth = i;
                                ...
                                if (null == img) {
                                    img = new ZImage(t,this.libObj["fileName"],cfo.libName, this.isBodyFlag);
                                    img.img_depth = i;
                                    img.touchEnabled = false;
                                    l_obj.img = img;
                                } else {
                                    img.setTexture(t,this.libObj["fileName"],cfo.libName, this.isBodyFlag);
                                }
