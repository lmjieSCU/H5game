...
(function (zmovie) {
    ...
    var WaitingCallScript = (function () {
        function WaitingCallScript(param0, param1, param2, param3) {
            this.mc = param0;
            this.funname = param1;
            this.fun = param2;
            this.params = param3;
        }
        return WaitingCallScript;
    }());
    __reflect(WaitingCallScript.prototype, "WaitingCallScript");
    var ZMovieClip = (function (_super) {
        ...
        function ZMovieClip(imgArr, libObj, mcName, scale_, _name) {
            ...
            _this._eventActFrame = -2; //最近发生帧事件的帧
            _this._nextFrameStop = false; //不允许nextFrame动作
            //
            _this.frameFuncArray = [];
            _this.WaitingCallFunArray = [];
            _this._name = "";
            _this.frameLabel = "";
            _this.isPlay = false;
            _this._name = _name;
        ...
        ZMovieClip.prototype.getchildMovieBymcName = function (mcName) {
            for (var i = 0; i < this.$children.length; i++) {
                if (this.$children[i] instanceof ZMovieClip) {
                    if (this.$children[i]._mcName == mcName) {
            ...
            if (typeof (frame) == "string") {
                ...
                for (var i = 0; i < this.$children.length; i++) {
                    if (this.$children[i] instanceof zmovie.ZMovieClip) {
                        this.$children[i]._nextFrameStop = false;
                        this.$children[i].gotoAndStop(1);
                    }
                }
        ...
        ZMovieClip.prototype.gotoAndStopNum = function (frame) {
            this.stopZMovie();
        ...
        ZMovieClip.prototype.addWaitingCallFun = function (mc, param) {
            for (var i = 0; i <= param.length - 3; i = i + 3) {
                var frameIndex1 = param[i];
                var fun1 = param[i + 1];
                var params = param[i + 2];
                var foundFlag = false;
                for (var j = 0; j < this.WaitingCallFunArray.length; j++) {
        ...
        /**true表示已寻到该函数且执行 */
        ZMovieClip.prototype.checkWaitingCallFun = function (_funname) {
            for (var i = 0; i < this.WaitingCallFunArray.length; i++) {
                if (this.WaitingCallFunArray[i].funname == _funname) {
                    (_a = this.WaitingCallFunArray[i].fun).call.apply(_a, [this.WaitingCallFunArray[i].mc].concat(this.WaitingCallFunArray[i].params));
                    return true;
                }
            }
            return false;
        ...
        ZMovieClip.prototype.addFrameScript = function (mc, param) {
            ...
            for (var i = 0; i <= param.length - 2; i = i + 2) {
                var frameIndex1 = param[i] - 1;
                var fun1 = param[i + 1];
                var foundFlag = false;
                for (var j = 0; j < this.frameFuncArray.length; j++) {
        ...
        ZMovieClip.prototype.checkFrameEvent = function () {
            var checkFrame = this.currFrame;
        ...
        ZMovieClip.prototype.resetFrameStop = function () {
            //恢复_nextFrameStop为false
            this._nextFrameStop = false;
        ...
        ZMovieClip.prototype.stopZMovie = function () {
            if (this.isPlay) {
                this.isPlay = false;
                    ...
                    try {
                        ...
                        if (null == cfo.libName) {
                            ...
                            if (null == t) {
                                ...
                                if (null == img) {
                                    img = new zmovie.ZImage(t, this.libObj["fileName"], cfo.libName, true);
                                    img.img_depth = i;
