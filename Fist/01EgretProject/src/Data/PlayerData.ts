class PlayerData {
    ...
    public static getRankReward(index: number) {//获取段位奖励的状态
        const status = egret.localStorage.getItem(`rankReward${index}`);
        if(status) {
            return status;
        } else {
    ...
    public static setRankReward(index: number, status: number) {
    ...
    public static setPlayerLv(Exp: number = 0, Lv: number = 0) {//修改等级和经验，当传值都为0时，只返回等级经验
        let lv = Number(egret.localStorage.getItem("playerLv")) || 1;
        let exp = Number(egret.localStorage.getItem("playerExp")) || 0;
        if(Lv!=0) lv += Lv;
        if(exp!=0 && lv<30) {
            let exper = exp + Exp;
