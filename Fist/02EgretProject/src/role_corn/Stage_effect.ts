class Stage_effect {
    public bgEff_mc1: any;
    public bgEff_mc2: any;
    public skill_loader: Array<any> = [];
    public eff_vs: Array<any> = [];
    public bgEffKO_mc: any;
    public camera_obj: any = null;
    public element_mc: any;
    public static zoom_number: number = 0;
    public static effectContainer: egret.DisplayObjectContainer;//添加全屏背景层
    public static superface: any;//人物头像
    public static freezeTime: number;//暂停多少帧
    public static shakedir: number;
    public static showAnimation: egret.DisplayObjectContainer = null;
    public static eff_num: number = 0;
    //渲染打击冻结
    public render_hitFreeze_action: Function;
    public render_hitFreeze_params: Array<any>;
    ...
    //打击特效
    public hitFire(_hitType: string, hitFire_x: number, hitFire_y: number, role: Role) {
        if (SplashScene.CloseEffects) {
            return;
        }
        let num = SplashScene.hero_layer.globalToLocal(hitFire_x, hitFire_y);
        var _loc4;
        var _loc2 = _hitType;
        ...
        let skill_class = eval(role._roleName + "_Skill");
        let skill = new skill_class(_loc2, role);
    ...
    //着地特效
    public downEff(hitType: string, role: Role) {
        if (SplashScene.CloseEffects) {
            return;
        }
        let skill_class = eval(role._roleName + "_Skill");
        let skill = new skill_class(hitType, role);
    ...
    //全屏背景
    public bgEff_start(s_type: string, role: Role) {
        if (SplashScene.CloseEffects) {
            return;
        }
        let bgEff_mc;
        ...
        let scaleX = GameConfig.FightWidth / bgEff_mc.skill_mc.measuredWidth;
        let scaleY = UI.WINSIZE_H / bgEff_mc.skill_mc.measuredHeight;
        let _x = 0;
    ...
    public static onbgEff_end(role: Role, owner: Stage_effect) {
    ...
    //屏幕闪耀
    public bgShineHit(type: number, role: Role) {
        if (SplashScene.CloseEffects) {
            return;
        }
        let skill_class = eval(role._roleName + "_Skill");
        let skill = new skill_class("bgShinngHit", role);
        ...
        egret.Tween.get(skill.skill_mc, { loop: false })
            .to({ alpha: 0 }, 50).call(() => { skill.skill_mc.parent.removeChild(skill.skill_mc) })
    } // End of the function

    public fgEff_start(s_type, fromMC) {
        // if (s_type == "")
        // {
        //     this.fgEff_end();
        //     return;
    ...
    //技能全屏效果
    public superStart(s_type: string, time_f: number, dx: number, dy: number, bgType: string, roleName: string, dir: number, fromMC: any, role: Role) {
        if (SplashScene.CloseEffects) {
            return;
        }
        if (bgType == undefined) {
        ...
        //背景  
        if (bgType != "") {
            let skill3 = new skill_class("bgEff_super" + bgType, role);
        ...
        if (roleName != "") {
            ...
            let path = "superface_" + roleName + bgType + "_png";
            ...
            if (bgType == "1") {
                ...
                let scaleX = UI.WINSIZE_H / image.measuredHeight;
                let scaleY = UI.WINSIZE_H / image.measuredHeight;
                if (dir == -1) {
                    image.skewY = 180;
                    image.x = GameConfig.FightWidth / 2 + image.measuredWidth;
                }
        ...
        //蓄力效果
        if (s_type != "") {
            let skill = new skill_class(s_type, role);
            ...
            //蓄力特效2
            if (s_type == "s1") {
                let skill2 = new skill_class(s_type + "_bg", role);
    ...
    //暂停多少帧
    public superFreeze(f: number, role: Role) {
    ...
    public render_superFreeze(skill_loader: Array<any>, role: Role) {
        if (_level0.inPause) {
            return;
        } // end if
        if (Stage_effect.freezeTime <= 4) {
            if (Stage_effect.superface != null) {
                let str = Stage_effect.superface["type"];
                if (str == "1") {
    ...
    public animation(superface: egret.Bitmap, role: Role) {
        if (Stage_effect.showAnimation == null) {
            ...
            for (let i = 1; i <= 6; i++) {
                let eff: egret.Bitmap = UI.addPic(Stage_effect.showAnimation, "eff_vs_bg000" + i + "_png", 0, 0, false);
    ...
    public upAnimation() {
        if (Stage_effect.eff_num >= this.eff_vs.length) Stage_effect.eff_num = 0;
    ...
    //KO特效
    public KO_Freeze(time_f: number, KO_type: number, role: Role) {
        // if (isKO)
        // {
        //     return;
        // } // end if
    ...
    public KO_onEnterFrame(time_f: number, role: Role) {
        if (_level0.inPause) {
            return;
        } // end if
    ...
    //攻击目标后冻结帧数
    public hitFreeze(freezeTime: number, slowTime, role: Role) {
        if (SplashScene.CloseEffects) {
            return;
        }
        if (role.isKO) {
    ...
    public render_hitFreeze(role: Role) {
        if (_level0.inPause) {
            return;
        } // end if
        if (role.isKO) {
    ...
    public KO_camera(role: Role) {
        let _loc3 = (_level0.P1_role.x + _level0.P2_role.x) / 2;
        let _loc2 = (_level0.P1_role.y - 100 + (_level0.P2_role.y - 100)) / 2;
        let skill_class = eval(role._roleName + "_Skill");
    ...
    public KO_eff(KO_type, role: Role) {
        let _loc3;
        let skill_class = eval(role._roleName + "_Skill");
        if (KO_type == "2") {
            _loc3 = "bgEff_KO2";
    ...
    //放大多少
    public static zoomControl_mc(zoom_number: number, role: Role) {
        Stage_effect.zoomControl_mc_primary();
    ...
    //返原大小
    public static zoomControl_mc_primary() {
    ...
    public renderAnimate(owner) {
        for (let index in owner.skill_loader) {
        ...
        if (owner.render_hitFreeze_action != undefined) {
            owner.render_hitFreeze_action.call(owner, ...owner.render_hitFreeze_params);
        }
    }
