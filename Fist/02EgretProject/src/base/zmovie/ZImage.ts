        ...
        public setTexture(t:egret.Texture,heroResName:string=null,picName:string=null, isBodyFlag2:boolean):void{
            ...
            if(heroResName.substr(0,4) == "role"){
                ...
                while (RES.getRes(`${heroResName}00${i}_json`)) {
                    ...
                    // else if(RES.getRes(`effect_${heroResName.substring(4).toLocaleLowerCase()}000_josn`)) {
                    //     this.img.texture = RES.getRes(`effect_${heroResName.substring(4).toLocaleLowerCase()}000_josn`).getTexture(`effect_${heroResName.substring(4).toLocaleLowerCase()}_${picName}`);
                    //     isChange = true;
                    //     break
                    // }
                    i++;
                }
                // if ( RES.getRes(heroResName+"000_json")&& RES.getRes(heroResName+"000_json").getTexture(heroResName + "_" + picName)){
                //     let i = 0;
                //     while(RES.getRes(heroResName))
                //     this.img.texture = RES.getRes(heroResName+"000_json").getTexture(heroResName + "_" + picName);
                //     this.img.anchorOffsetY = 10;
                //     this.img.anchorOffsetX = 10;
                // if ( RES.getRes(heroResName + "_" + picName + "_png")){
                //     this.img.texture = RES.getRes(heroResName + "_" + picName + "_png");
                //     this.img.anchorOffsetY = 10;
                //     this.img.anchorOffsetX = 10;
                        // RES.getRes(heroResName+"000_json").getTexture(heroResName + "_" + picName)
                        // RES.getRes(heroResName + "_" + picName + "_png")
                    //
