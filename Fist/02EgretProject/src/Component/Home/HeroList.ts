class HeroList extends egret.DisplayObjectContainer implements egret.DisplayObject {
    public constructor(parent: any, x: number, y: number, wid: number, high: number) {
        super();
    ...
    onEnter() {
        ...
        // this.heroScroll.$setWidth(this.width);
        // this.heroScroll.$setHeight(this.height);

        // this.heroScroll.horizontalScrollPolicy = 'on';

        // this.heroScroll._onTouchEnd = this.moveHero;
    ...
    update() {
        if(this.now >= this.max && this.heros) {
            ...
            // let img = this.heros.getChildAt(this.nowHero) as egret.Bitmap;
            // img.$setTexture(RES.getRes("stand_json").getTexture(`${hero.id}_i_${hero.render[this.nowRender]}`));
