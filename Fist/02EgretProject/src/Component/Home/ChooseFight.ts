class ChooseFight extends egret.DisplayObjectContainer implements egret.DisplayObject {
    ...
    onEnter() {
        if(Main.UItype == Main.UIT2) {
            ...
            bg.x = bg.width/2;
            bg.y = bg.height/2;
            let shilian: egret.Bitmap = UI.addBtn(this, "aiBtn_png", bg.width*0.125, bg.height*0.3125, true, () => this.goToFightReady('level'));
            let pipei: egret.Bitmap = UI.addBtn(this, "fightBtn_png", bg.width*0.85, bg.height*0.3125, true, () => {
    ...
    public goToFightReady(type: string) {
        ...
        SceneManager.getInstance().RemoveLayer(this.$parent as Scene);
        SceneManager.getInstance().AddLayer(new FightReady(type, ((this.parent as HomeLayer).getChildAt(1) as HeroList).getHeroName()));
        Home.fightType = type;
