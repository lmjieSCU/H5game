class Kyo_AI extends Kyo_role
{
    ...
    {
        var _loc2 = int(Math.random() * 100);
        if (act == "sk31" && _loc2 < attack_s && inBurst && burst_mode == "attack")
        {
            skill_next = this.selectOne(["sk29", "sk3", "sk0"]);
            this.combo_start();
        }
        else if (act == "sk28" && _loc2 < attack_s && inBurst && burst_mode == "attack")
        {
            skill_next = this.selectOne(["sk0", "sk1", "heavyHit", "sk29", "sk31", "sk3"]);
            skill_next = this.selectOne(["sk0", "sk1", "heavyHit", "sk29", "sk31", "sk3"]);
            this.combo_start();
        } // end else if
    } // End of the function
