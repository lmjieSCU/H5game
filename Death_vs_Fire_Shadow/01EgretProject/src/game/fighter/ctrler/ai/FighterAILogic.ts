    ...
    class FighterAILogic extends FighterAILogicBase
    {
        ...
        private _moveKeep:number = 30;
        private _catchMoveKeep:number = 10;
        private _dashKeep:number = 200;
        private _jumpKeep:number = 50;
         ...
         public render() 
        {
         ...
         protected updateActionAI()         //负责攻击相关的命令
        {
            this.updateDashAI();
            this.updateAttackAI();
            this.updateSkill();
        ...
        private updateMoveAI() 
        {
            ...
            //不处于防御状态
            if (this._moveFrame < GameConfig.FPS_GAME)//可移动总帧数为GameConfig.FPS_GAME
            {
                this._moveFrame = this._moveFrame + 1;
                _loc_3 = true;
            }
            else    //每经过一轮ameConfig.FPS_GAME则重新判断_moveFrame的取值
            {
                _loc_5 = {};
            ...
            if (!this.moveLeft && !this.moveRight)      //均为false时
            {
        ...
        private updateJumpAI() 
        {
            var _loc_1 = {};
            if (this._isConting)    //在攻击时
            {
                _loc_1["defult"] = [1, 2, 3, 2, 1, 0];
                _loc_1[21] = [0, 1, 2, 3, 3, 4];    //21 = HURT_ING
            }
            else if (this._fighter.y > this._target.gety() + this._jumpKeep)    //目标在自己上方
            {
            ...
            if (this._isConting && this.jump)   //如果正在攻击,又做出了jump = true的判断则将jump命令存入ContOrder
            {
        ...
        private updateJumpDownAI() 
        {
            var _loc_1 = {};
            if (this._fighter.y < this._target.gety() - this._jumpKeep) //AI在对方上方,应有较大几率下跳
            {
                _loc_1["defult"] = [2, 3, 4, 5, 6, 6];
                _loc_1[12] = [2, 1, 0, 0, 0, 0];
                _loc_1[13] = [2, 1, 0, 0, 0, 0];
            }
            else                                                //AI在对方下方,应有较小几率下跳
            {
                _loc_1["defult"] = [0.01, 0, 0, 0, 0, 0];
        ...
        private updateDefenseAI() 
        {
            var _loc_1 = {};
            var _loc_5 = null;
            var _loc_3 = null;
            if (this.defense)       //正在防御
            {
            ...
            for  (var _loc_7 in _loc_4)
            {
                ...
                {
                    if (_loc_4[_loc_7] instanceof Bullet && (_loc_4[_loc_7] as Bullet).isAttacking())
                    {
                    ...
                    if (_loc_4[_loc_7] instanceof FighterAttacker && (_loc_4[_loc_7] as FighterAttacker).isAttacking)
                    {
                    ...
                    if (_loc_4[_loc_7] instanceof Assister && (<Assister>_loc_4[_loc_7]).isAttacking)
                    {
        ...
        private getSkillAI(param1:string, param2:string, param3:string, param4:number) : boolean
        {
            var _loc_6 = {};
            var _loc_5 = false;
            if (this.isBreakAct(param2))        //可以打破防御,则在对方防御时有较大几率释放该技能
            {
        ...
        private getBishaAI(param1:string, param2:string, param3:string, param4:number, param5:number) : boolean
        {
            var _loc_7 = {};
            var _loc_6 = false;
        ...
        private updateSpecialSkill()    //暂时用不上
        {
            var _loc_1 = null;
            if (this._fighter.actionState == 21)    //21 = HURT_ING,被打
            {
                _loc_1 = {};
        ...
        private updateGhostStep() 
        {
            var _loc_4 = null;
            var _loc_2 = null;
            var _loc_3 = this.getTargetDistance(this._target);
            if (_loc_3.x > 80 || _loc_3.y > 80) //距离太远不使用瞬步
            {
                return;
            }
            if (this._fighter.actionState != 10 && this._fighter.actionState != 11) //ATTACK_ING 和 skilling时才可能使用
            {
                return;
            }
            var _loc_1 = {};
